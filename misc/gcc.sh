...
CloneCompiledGccEleven(){
    ...
    mkdir "${GCCaPath}"
    mkdir "${GCCbPath}"
    if [ ! -e "${MainPath}/arm-zyc-linux-gnueabi-11.x-gnu-20210808.tar.gz" ];then
        wget -q  https://github.com/ZyCromerZ/compiled-gcc/releases/download/varm-zyc-linux-gnueabi-11.x-gnu-20210808/arm-zyc-linux-gnueabi-11.x-gnu-20210808.tar.gz
    fi
    tar -xf arm-zyc-linux-gnueabi-11.x-gnu-20210808.tar.gz -C $GCCbPath
    GCCbPath="${GCCbPath}/arm-zyc-linux-gnueabi"
    for32=arm-zyc-linux-gnueabi
    if [ ! -e "${MainPath}/aarch64-zyc-linux-gnu-11.x-gnu-20210808.tar.gz" ];then
        wget -q https://github.com/ZyCromerZ/compiled-gcc/releases/download/vaarch64-zyc-linux-gnu-11.x-gnu-20210808/aarch64-zyc-linux-gnu-11.x-gnu-20210808.tar.gz
    fi
    tar -xf aarch64-zyc-linux-gnu-11.x-gnu-20210808.tar.gz -C $GCCaPath
